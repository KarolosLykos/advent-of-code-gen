...
const testTemplate = `// Code generated by aocgen; DO NOT EDIT.
package main
...
func genCmd(cmd *cobra.Command, _ []string) error {
	...
	if err = createTestFile(dayDir, yearFlag, dayFlag); err != nil {
		logrus.Error(err)
...
func createTestFile(dayDir string, yearFlag, dayFlag int) error {
	_, err := os.Stat(dayDir + "/main_test.go")
	if err == nil && !errors.Is(err, os.ErrNotExist) {
		logrus.Infof("test for solution exists")

		return nil
	}

	tmpl := codegen.MustParse(testTemplate)

	return tmpl.CreateFile(dayDir+"/main_test.go", map[string]interface{}{"Year": yearFlag, "Day": formatDay(dayFlag)})
}
